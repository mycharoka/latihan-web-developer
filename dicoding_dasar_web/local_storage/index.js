...
function checkAnswer(userGuess) {
  ...
  if (userGuess === answer) {
    ...
    const previousAttemptTotal = parseInt(
      sessionStorage.getItem(sessionUserAttemptsKey),
    );

    sessionStorage.setItem(sessionUserAttemptsKey, previousAttemptTotal + 1);
    sessionUserAnswerField.innerText = sessionStorage.getItem(
      sessionUserAttemptsKey,
    );
...
function updateScore() {
  ...
  localStorage.setItem(localTotalVictoryKey, previousAttemptTotal + 1);
  localTotalVictoryField.innerText = localStorage.getItem(localTotalVictoryKey);
}
...
destroyDataButton.addEventListener("click", () => {
  ...
  alert("Refresh kembali halaman ini");
});
